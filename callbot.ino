...
#define TELEGRAM_BOT_TOKEN "xxxxxxxx"
...
#define LED_PIN 2 // For Announce a Call.
#define LED_PIN2 0 // For Announce I'm using Headphones.
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (bot.messages[i].type ==  F("callback_query")) {
      ...
      if (text == F("ON")) {
        digitalWrite(LED_PIN, HIGH);      // Turn on the led for a Call
      } else if (text == F("BT")) {
        digitalWrite(LED_PIN2, HIGH);     // Turn on the led for BT Headphones
      } else if (text == F("OFF")) {
        digitalWrite(LED_PIN, LOW);       // Turn off the led for a Call
      } else if (text == F("BTOFF")) {
        digitalWrite(LED_PIN2, LOW);      // Turn off the led for BT Headphones
      } else if (text == F("ALLOFF")) {
        digitalWrite(LED_PIN, LOW);       //Turn off both leds
        digitalWrite(LED_PIN2, LOW);
      } else if (text.startsWith("TIME")) {
        text.replace("TIME", "");        // This option Set a timer for the led for a Call.
      ...
      if (text == F("/options")) {
        ...
        String keyboardJson = F("[[{ \"text\" : \"ON\", \"callback_data\" : \"ON\" },{ \"text\" : \"OFF\", \"callback_data\" : \"OFF\" },{ \"text\" : \"BT\", \"callback_data\" : \"BT\" },{ \"text\" : \"BTOFF\", \"callback_data\" : \"BTOFF\" },{ \"text\" : \"ALLOFF\", \"callback_data\" : \"ALLOFF\" }],");
        keyboardJson += F("[{ \"text\" : \"10 Mins\", \"callback_data\" : \"TIME10\" }, { \"text\" : \"20 Mins\", \"callback_data\" : \"TIME20\" }, { \"text\" : \"30 Mins\", \"callback_data\" : \"TIME30\" }]]");
        bot.sendMessageWithInlineKeyboard(chat_id, "Rafaman's Call Status", "", keyboardJson);
      }
      ...
      if (text == F("/start")) {
        ...
        bot.sendMessage(chat_id, "/options : Lista the opciones\n", "Seleccione");
      }
    }
  }
}
